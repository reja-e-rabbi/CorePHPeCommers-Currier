...
$col = " SL,img,proname,quantity,price,info,category,subcategory,location,totalview,totalsels,shopName,dates,productID,statuses,country,State,SubState,P_state,Activity,coID";
?>
<?php
switch ($SOPS) {
  case 'RECOMMENDED':
    ...
    <div class="recomand flex-col">
            ...
            while ($stmt->fetch()) {
                $arrCt = array("img"=>$img,"proname"=>$proname,"proID"=> $productID,"coID"=>$coID,"price"=>$price,"quantity"=>$quantity,"category"=>$category,"SubCat"=>$subcategory,"location"=>$location.','.$SubState.','.$State,"ShopName"=>$shopname);
                $arrcatJson = json_encode($arrCt);
                $arrCom = array("img"=>$img,"proname"=>$proname,"info"=>$info,"location"=>$location,"shopName"=>$shopname,"proID"=> $productID,"coID"=>$coID,"price"=>$price,"quantity"=>$quantity,"category"=>$category,"SubCat"=>$subcategory);
